 ...
 */
public class TablaModelAnnoCurso extends AbstractTableModel {

    private ArrayList<String> headerTable = new ArrayList();
    private ArrayList<AnnoCurso> data = new ArrayList();
    private ArrayList<TableModelListener> listeners = new ArrayList();
    private JTable tablaDatos;
    private HashSet<AnnoCurso> listaCommit = new HashSet();

    public TablaModelAnnoCurso(JTable tabla) {
        tablaDatos = tabla;
        createHeaders();
    }

    public TablaModelAnnoCurso(JTable tabla, ArrayList<AnnoCurso> lista) {
        tablaDatos = tabla;
        data = lista;
        createHeaders();
    }
    ...
    public void insertar(AnnoCurso i) {
        data.add(i);
        TableModelEvent evento;
    ...
    public void update(AnnoCurso i, int fila) {
        data.remove(fila);
        data.add(fila, i);
        TableModelEvent evento;
    ...
    public AnnoCurso getValue(int rowIndex) {
        try {
    ...
    public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
        ...
        String valor;
           BigDecimal valor1;
        AnnoCurso auxObj = data.get(rowIndex);
        boolean eliminar = false;
        switch (columnIndex) {
            case 0:
                break;
            case 1:
                 valor1 = (BigDecimal)aValue;
                if (!data.get(rowIndex).getEstadoOperacion().equals("i")) {
                    aux = String.valueOf(data.get(rowIndex).getId());
                    data.get(rowIndex).setId(valor1);
                    if (aux.equals(valor1.toString())) {
            ...
            case 3:
                valor = String.valueOf(aValue);
                if (!data.get(rowIndex).getEstadoOperacion().equals("i")) {
                    aux = String.valueOf(data.get(rowIndex).getEstadoActual());
                    data.get(rowIndex).setEstadoActual(valor);
                    if (aux.equals(valor)) {
                        data.get(rowIndex).setEstadoOperacion("n");
                    } else {
                        data.get(rowIndex).setEstadoOperacion("a");
        ...
        try {
            if (!eliminar) {
                ...
                while (iterar.hasNext()) {
                    AnnoCurso bean = (AnnoCurso) iterar.next();
    ...
    public ArrayList<AnnoCurso> getData() {
        return data;
    }

    public void setData(ArrayList<AnnoCurso> data) {
        this.data = data;
        ...
        tablaDatos.getColumnModel().getColumn(3).setCellEditor(new CellEditorAnnoCurso(data));
          tablaDatos.getColumnModel().getColumn(3).setCellRenderer(new CellRendererImageEstado());
    ...
    public HashSet<AnnoCurso> getListaCommit() {
        return listaCommit;
    }
}
