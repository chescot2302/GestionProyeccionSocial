 ...
 */
public class TableModelUbigeo extends AbstractTableModel {

    private ArrayList<String> headerTable = new ArrayList();
    private ArrayList<Ubigeo> data = new ArrayList();
    private ArrayList<TableModelListener> listeners = new ArrayList();

    public TableModelUbigeo() {
        createHeaders();
    }

    public TableModelUbigeo(ArrayList<Ubigeo> lista) {
        data = lista;
        createHeaders();
    }
    ...
    public void updateo(Ubigeo obj, int fila) {
        data.remove(fila);
        data.add(fila, obj);
        TableModelEvent evento;
    ...
    public Ubigeo getValue(int rowIndex) {
        try {
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        switch (columnIndex) {
            ...
            case 1:
                return data.get(rowIndex).getDepartamento();
            case 2:
                return data.get(rowIndex).getProvincia();
            case 3:
                return data.get(rowIndex).getDistrito();            
            default:
                return null;
        }
    }
